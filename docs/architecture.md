...
Inbound channels (Telegram, Discord) arrive as HTTPS webhooks / WebSocket maintained by the gateway. The gateway never has a public port — channels are outbound-initiated connections or Telegram long-polling.

---

## Agents

Three isolated agents, each with its own workspace and session store:
...
```json5
{
  ...
  bindings: [
    { match: { channel: "telegram", peer: { kind: "dm", id: "${YOUR_TG_ID}" } },    agentId: "main" },
    { match: { channel: "telegram", peer: { kind: "dm", id: "${HONEY_TG_ID}" } },   agentId: "honey" },
...
Models are injected from `.env` — swapping a tier requires an env change + `openclaw daemon restart`.
